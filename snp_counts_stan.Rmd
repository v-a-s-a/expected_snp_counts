...
```{r}
meta_df <- read_tsv('PGC_chip_QC_fields-good_qc_runs.tsv') %>% 
...
plat_annot <-  read_tsv('PGC chip QC - qc_runs_round2.tsv', col_types = cols()) %>% 
...
fit <- stan(file = 'fit_nc_snp_counts.stan',
            data = stan_data,
            chains = 1,
            iter = n_iter,
            warmup = n_warmup)
...
y_tilde <- extract(fit) %>%
  pluck('y_tilde') %>% c() %>% 
  as_tibble() %>%
  mutate(iter = rep(1:nrow(data_df), n_iter - n_warmup))
...
new_data <- list(X = platform_onehot %>% c(),
                 ...
                 mu = extract(fit) %>% pluck('mu') %>% unlist(),
                 sigma = extract(fit) %>% pluck('sigma') %>% unlist())
predictions <- stan(file = 'predict_snp_counts.stan',
            data = new_data,
...
for (plat in unique(data_df$platform)) {
  ...
  eval_metrics <- tibble(y_tilde = extract(predictions) %>% pluck('y_tilde') %>% c(),
                       y_loglik = extract(predictions) %>% pluck('y_loglik') %>% c()) %>%
